...
class DashDownloader:
    ...
    def __init__(self, mpd_url, referrer, target_mpd, user_agent=None):
        """
        ...
        self.target_dir = os.path.split(target_mpd)[0]
    ...
    def process_segments(self, stop_event):
        ...
        last = {}  # Mapping from init-file to last known segment
        ...
        while not stop_event.is_set():
            ...
            for segment in segments:
                ...
                if segment["init_url"] in last and segnum - last[segment["init_url"]]["segnum"] > 1:

                    last_segment = last[segment["init_url"]]
                ...
                if not segment["init_url"] not in last:
                    offset = segment["startts"] - segment["segnum"]
                    duration = segment["endts"] - segment["startts"]

                fn = self.download_segment(segment["url"], segment["segnum"], os.path.join(self.target_dir, segment["dst"]))
                last[segment["init_url"]] = segment
    ...
    def notify_video(self, segment, filename):
        ...
        for q in self.video_queues:
            if "quality" in segment and segment["quality"].startswith("640"):
                s = copy.copy(segment)
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if last_seg and segment["segnum"] == last_seg:
                print("   **** Error - already processed this segment")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--dst", dest="dst", help="Output file (mpd)", required=True)
    ...
    downloader = DashDownloader(options.src, options.referrer, options.dst)
    ...
    # DAR analyzer
    aux_dst = os.path.splitext(options.dst)[0] + "_aux.json"
